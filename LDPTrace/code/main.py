...
import trajectory
import ldp
from grid import GridMap, Grid, GridVisualizer
import map_func
import utils
import experiment
...
class MarkovMatrixVisualizer:
    def __init__(self, markov_matrix, grid_map_size):
        self.markov_matrix = markov_matrix
        self.grid_map_size = grid_map_size
    
    def visualize_markov_matrix(self):
        plt.figure(figsize=(14, 12))  # Increased figure size for better readability
        
        plt.imshow(self.markov_matrix, cmap='viridis', interpolation='none')
        plt.colorbar(label='Transition Probability')
        
        plt.title(f'Markov Matrix (Grid Size: {self.grid_map_size}x{self.grid_map_size})')
        plt.xlabel('Grid Index (To)')
        plt.ylabel('Grid Index (From)')
        
        # Limit the number of ticks to prevent cluttering
        num_ticks = 10 if self.grid_map_size > 10 else self.grid_map_size
        ticks = np.linspace(0, self.grid_map_size, num_ticks, dtype=int)
        plt.xticks(ticks)
        plt.yticks(ticks)
...
class TransitionMatrixVisualizer:
    def __init__(self, grid_map, transition_matrix):
        self.grid_map = grid_map
        self.transition_matrix = transition_matrix

    def aggregate_transitions(self):
        n = self.grid_map.size
        grid_size = int(np.sqrt(n))
        transition_heatmap = np.zeros((grid_size, grid_size))

        for i in range(grid_size):
            for j in range(grid_size):
                # The sum of all transitions leading to the grid cell (i, j)
                transition_heatmap[i, j] = np.sum(self.transition_matrix[:, i * grid_size + j])

        return transition_heatmap
    ...
    def visualize_transition_heatmap(self):
        ...
        for i in range(len(self.grid_map.map)):
            for j in range(len(self.grid_map.map[i])):
                ...
                rect = plt.Rectangle((j, i), 1, 1, fill=None, edgecolor='black')
                ax.add_patch(rect)
                ax.text(j, i, f'{grid.index}', ha='center', va='center', fontsize=8, color='white')

        ax.set_xticks(np.arange(0, transition_heatmap.shape[1], 1))
        ax.set_yticks(np.arange(0, transition_heatmap.shape[0], 1))
        ax.set_xticklabels(np.arange(1, transition_heatmap.shape[1] + 1))
        ax.set_yticklabels(np.arange(1, transition_heatmap.shape[0] + 1))

        plt.xlabel('Grid X')
        plt.ylabel('Grid Y')
        plt.title('Transition Heatmap (GridxGrid)')
        plt.show()
...
# Function to filter trajectories with distinct start and end grids
def filter_distinct_start_end_trajectories(grid_trajectories):
    distinct_trajectories = []
    for trajectory in grid_trajectories:
        if len(trajectory) > 1 and trajectory[0].index != trajectory[-1].index:
            distinct_trajectories.append(trajectory)
    return distinct_trajectories

# Filter the trajectories from your dataset
distinct_trajectories = filter_distinct_start_end_trajectories(grid_trajectories)

# Visualize all the trajectories from your dataset
if grid_trajectories:
    visualizer = GridVisualizer(grid_map, db)
    visualizer.visualize_raw_trajectories()
else:
    print("No trajectories found in the dataset.")
...
if args.re_syn:
    ...
    logger.info('Visualizing Markov Matrix...')
    markov_visualizer = MarkovMatrixVisualizer(one_level_mat, grid_map.size)
    markov_visualizer.visualize_markov_matrix()

    logger.info("Visualizing Transition Matrix...")
    transition_visualizer = TransitionMatrixVisualizer(grid_map, one_level_mat)
    transition_visualizer.visualize_transition_heatmap()
